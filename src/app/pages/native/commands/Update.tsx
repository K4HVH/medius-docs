...
+--------+--------+--------+--------+--------+--------+----------+--------+`}</pre>
     ...
     +--------------------+--> device commits, replies OK, reboots`}</pre>
          ...
          <div class="callout callout--warning">
            <p>
              <strong>A refused commit leaves the image staged.</strong> The host chip goes first, so
              if it refuses, the device chip never commits. Both chips are still on their running
              slots, but the device image stays staged and armed: the next <code>ACTIVATE</code> would
              commit it alone and leave the two chips on different versions. After a failed{' '}
              <code>ACTIVATE</code>, either retry the whole update or send <code>ABORT</code> for each
              staged target first. The dashboard and both reference clients do the latter for you.
            </p>
          </div>
          <div class="api-response-label">REPLY</div>
          <p>
          ...
          <p>
            Once <code>ACTIVATE</code> is accepted the box answers new commands with{' '}
            <code>BUSY</code>. <code>ABORT</code> is the one exception, and only while the box is
            waiting on the host chip: it abandons that wait and answers the <code>ACTIVATE</code> with{' '}
            <code>TIMEOUT</code>. Past that point the boot partition is already written, so there is
