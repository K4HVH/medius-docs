...
const Catch: Component = () => {
  return (
    <>
      <Card>
        <CardHeader title="Catch" subtitle="Stream the physical mouse's input" />
        <p>
          <A href="/library/catch#catch-events"><code>catch_events</code></A> subscribes to the real
          mouse's input and hands back an <A href="/library/catch#event-stream"><code>EventStream</code></A>{' '}
          of <A href="/library/types/structs#input-report"><code>InputReport</code></A> snapshots —
          buttons, wheel, and movement as the user makes them, captured before any{' '}
      ...
      <div id="catch-events" data-search-target>
        <Card>
          ...
          <p>
            ...
            combined with <code>|</code>, or <code>CatchMask::all()</code> for the full mirror. The
            returned <A href="/library/catch#event-stream"><code>EventStream</code></A> receives every
            event; the subscribe itself sends one frame and doesn't wait for a reply.
          </p>
          ...
          <table class="api-params">
            ...
            <tbody>
              <tr><td><code>mask</code></td><td><A href="/library/types/structs#catch-mask"><code>CatchMask</code></A></td><td>Which classes to stream. <code>MOTION</code> / <code>WHEEL</code> / <code>BUTTONS</code> or <code>all()</code>.</td></tr>
            </tbody>
          </table>
          <p>
          ...
          <pre><code>{`use medius::{Device, CatchMask, Button};

let device = Device::find()?;
let events = device.catch_events(CatchMask::all())?;   // or MOTION | BUTTONS
      ...
      <div id="event-stream" data-search-target>
        <Card>
          ...
          <p>
            ...
            Pull <A href="/library/types/structs#input-report"><code>InputReport</code></A> snapshots
            with whichever method fits your loop; cloning shares the queue (like{' '}
          ...
          <table class="api-params">
            ...
            <tbody>
              <tr><td><code>recv()</code></td><td><code>Result&lt;InputReport&gt;</code></td><td>Block until the next report.</td></tr>
              <tr><td><code>try_recv()</code></td><td><code>Option&lt;InputReport&gt;</code></td><td>The next buffered report, or <code>None</code> (never blocks).</td></tr>
              <tr><td><code>recv_timeout(dur)</code></td><td><code>Option&lt;InputReport&gt;</code></td><td>Block up to <code>dur</code>; <code>None</code> on timeout.</td></tr>
              <tr><td><code>try_iter()</code></td><td><code>impl Iterator</code></td><td>Drain every buffered report without blocking.</td></tr>
              <tr><td><code>recv_async().await</code></td><td><code>Result&lt;InputReport&gt;</code></td><td>Await the next report (<code>async</code> feature), runtime-agnostic.</td></tr>
