...
// So: watch for the disconnect, wait a beat, then make the user explicitly confirm
// USB1 is out too (USB1 + USB3 together can shut the machine down).
const DELAY_MS = 1500;

export const UnplugWatch = (props: { onUnplugged: () => void }) => {
  const [phase, setPhase] = createSignal<'waiting' | 'checking' | 'confirm'>('waiting');
  ...
  const spinnerRow = (label: string) => (
    <div style={{ display: 'flex', 'align-items': 'center', gap: 'var(--g-spacing-sm)' }}>
      ...
      <span style={{ color: 'var(--g-text-secondary)' }}>{label}</span>
    </div>
  );
  ...
  return (
    <Switch>
      <Match when={phase() === 'waiting'}>
        ...
        {spinnerRow('Waiting for the box to disconnect...')}
      ...
      <Match when={phase() === 'checking'}>{spinnerRow('Checking...')}</Match>
