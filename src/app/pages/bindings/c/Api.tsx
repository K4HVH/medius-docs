        ...
        <Card>
          ...
          <table class="api-params">
            ...
            <tbody>
              ...
              <tr><td><code>medius_device_set_emit_pace(MediusDevice *dev, uint8_t mode, uint16_t hz, uint16_t force_hz)</code></td><td>Pick the pace (<code>hz</code> is the target for <code>FIXED</code>) and the advertised rate (<code>force_hz</code>, 0 = the device's own). Both ride one frame; a <code>mode</code> no constant names is <code>MEDIUS_STATUS_ERR_INVALID_ARG</code> and nothing is sent. <code>full</code> is off on a box that has not been set. See <A href="/library/options">Options</A>.</td></tr>
        ...
        <Card>
          ...
          <table class="api-params">
            ...
            <tbody>
              ...
              <tr><td><code>medius_mock_set_render(mock, uint8_t mode, bool full, bool ready)</code></td><td>Set the texture the mock reports, whether the device's own motion goes through it, and whether a profile has armed. A mode no constant names leaves the texture alone.</td></tr>
