        ...
        <Card>
          ...
          <table class="api-params">
            ...
            <tbody>
              ...
              <tr><td><code>medius_mock_set_clip_status(mock, MediusClipStatus value) /</code> <code>_set_clip_settings(mock, MediusClipSettings value)</code></td><td>Set what the mock answers to the two clip queries. The settings' <code>packet_triggers</code> become the set <code>medius_clip_bind_packet</code> adds to, under the bounds the box holds it to.</td></tr>
              <tr><td><code>medius_mock_clip_packet(mock, uint8_t class_, uint16_t id, uint8_t direction,</code> <code>const uint8_t *head, uintptr_t head_len, uint8_t *out_action, bool *out_consumed)</code></td><td><code>bool</code>: run one packet through the mock's packet triggers, as the box does; the winner counts it in its <code>hits</code>. True when the winner runs its action on this packet, written to <code>*out_action</code>; false when no trigger wins, and when a <code>once_per_run</code> winner sees the packet continue a run. <code>*out_consumed</code> is whether the winner consumes it. A null out is skipped.</td></tr>
